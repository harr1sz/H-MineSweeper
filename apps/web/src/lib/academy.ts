...
const EN_CHAPTERS: Readonly<Record<AcademyChapterId, Omit<AcademyChapter, "id">>> = {
  0: { title: "Start with one clue", subtitle: "A clue counts mines in its eight neighbors.", description: "Learn the basic rules for finding certain mines and safe cells." },
  ...
  2: { title: "Shared and exclusive regions", subtitle: "Compare the covered-cell sets around two clues.", description: "Derive subset logic instead of memorizing 1-1 and 1-2 shapes." },
...
const EN_EXERCISE_COPY: Readonly<Record<string, AcademyExerciseCopy>> = {
  ...
  "c1-residual-mine": { title: "One mine remains", objective: "Subtract confirmed mines and find the remaining mine.", premise: "Clue 2 touches one confirmed mine and one covered cell.", proof: "2 minus one confirmed mine leaves one; A must be a mine.", hints: sixHints("Do not treat 2 as needing two more mines.", "Subtract the confirmed mine first.", "Remaining mines: 2 - 1 = 1.", "The only covered cell must contain it.", "Choose Mine, then select A.", "A is the remaining mine.") },
  "c1-residual-safe": { title: "No mines remain", objective: "Clear safe cells after a clue is satisfied.", premise: "Clue 2 already touches two confirmed mines.", proof: "2 minus two confirmed mines leaves zero; the other covered cell is safe.", hints: sixHints("Recalculate the clue after confirmed mines.", "Both required mines are already known.", "Remaining mines: 2 - 2 = 0.", "Every other covered neighbor is safe.", "Choose Safe and select the remaining covered cell.", "No additional mine can touch clue 2.") },
  ...
  "c3-pattern-121": { title: "1-2-1 with conditions", objective: "Work out the pattern from overlapping clues and check when it applies.", premise: "All relevant covered cells are shown, and confirmed mines have already been counted.", proof: "The outer cells are mines and the middle cell is safe under these 1-2-1 clues.", hints: sixHints("Check every covered neighbor before naming a pattern.", "Write each clue as a simple count.", "Compare the left and middle clues.", "Repeat on the right.", "Mark the outer targets Mine and the center Safe.", "The conclusion follows from the clue counts, not the visual shape alone.") },
  "c3-pattern-1221": { title: "1-2-2-1 with conditions", objective: "Use the overlapping clues to solve the four covered cells.", premise: "All relevant covered cells are shown, and confirmed mines have already been counted.", proof: "The two middle cells are mines and the two outer cells are safe.", hints: sixHints("Start from both end clues.", "Compare the two clues on the left.", "Compare the two clues on the right.", "Subtract the shared counts to isolate both middle cells.", "Mark the middle cells Mine and the outer cells Safe.", "The only valid order is safe, mine, mine, safe.") },
  ...
  "practice-unseen-transfer": { title: "Apply the logic to a new board", objective: "Use the same deductions when the board is rotated or surrounded by extra clues.", premise: "Check all covered neighbors before applying a familiar pattern.", proof: "The board's orientation changes, but the relationships between its clues and covered cells do not.", hints: sixHints("Set the orientation aside.", "List the covered cells beside each clue.", "Check for extra covered neighbors.", "Compare the resulting clue counts.", "Apply only the conclusion supported by those counts.", "Rotation changes coordinates, not logic.") },
  "practice-review-clinic": { title: "Review an uncertain move", objective: "Recognize when the visible clues do not yet determine a cell.", premise: "Exactly one of the two covered cells is a mine, but the visible clue cannot distinguish them.", proof: "Putting the mine on the left or on the right both satisfies clue 1, so neither cell is currently determined.", hints: sixHints("List the possible layouts without opening the final board.", "Mine on the left and safe on the right works.", "Safe on the left and mine on the right also works.", "Both layouts remain possible.", "Choose Currently undetermined for both cells.", "There are two valid layouts, so neither cell is forced.") },
...
export function formatAcademyProofTrace(
  ...
  const countNoun = (count: number, singular: string, plural = `${singular}s`) =>
  ...
  const constraintText = analysis.trace.constraints
    .map((constraint) => {
      ...
      return english
        ? `Clue ${constraint.clue} at row ${row}, column ${column}: after subtracting ${countNoun(constraint.knownMines, "confirmed mine")}, ${countNoun(constraint.remainingMines, "mine")} ${constraint.remainingMines === 1 ? "remains" : "remain"} across ${countNoun(constraint.unknownIndexes.length, "covered cell")}`
  ...
  const conclusionText = analysis.trace.conclusions
    ...
    .join(english ? ", " : "，");
  if (english) {
    return conclusionText.length > 0
      ? `${constraintText}. These visible constraints allow ${countNoun(analysis.solutionCount, "valid layout")}; ${conclusionText}.`
      : `${constraintText}. These visible constraints allow ${countNoun(analysis.solutionCount, "valid layout")}. No individual covered cell is determined in every valid layout.`;
  }
  return conclusionText.length > 0
    ? `${constraintText}。这些可见约束共有 ${analysis.solutionCount} 个合法布置；${conclusionText}。`
    : `${constraintText}。这些可见约束共有 ${analysis.solutionCount} 个合法布置；没有任何一个盖住格在全部合法布置中都保持同一结果。`;
}
