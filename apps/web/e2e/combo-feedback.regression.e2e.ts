...
function acceptedSafeRevealSequence(): readonly number[] {
  ...
  for (let index = 0; index < state.visibility.length && accepted.length < 4; index += 1) {
  ...
  if (accepted.length < 4) throw new Error("deterministic board lacks four accepted safe reveals");
  return accepted;
}
...
test("安全操作从 ×2 展示、每次续连重播脉冲，无效 reveal 清除连击", async ({ page }) => {
  await useDeterministicSoloEnvironment(page);
  await enterClassicSolo(page);
  const [first, second, third] = acceptedSafeRevealSequence();
